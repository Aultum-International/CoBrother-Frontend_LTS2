...
import ShareDropdownPortal from '../components/common/ShareDropdownPortal';
...
import { ListingSyncAction, ListingEntityType, matchesDomainListing } from '../utils/listingSync';
...
export default function DomainsDashboardPage() {
  ...
  useListingSync((detail) => {
    if (detail?.action === ListingSyncAction.DELETE
        && detail.entityType === ListingEntityType.DOMAIN
        && detail.id) {
      setListings((prev) => prev.filter((d) => !matchesDomainListing(d, detail.id)));
    }
    if (detail?.action === ListingSyncAction.INVALIDATE
        && (!detail.entityType || detail.entityType === ListingEntityType.DOMAIN)) {
      domainAPI.getMyListings()
        .then(({ data }) => setListings(Array.isArray(data) ? data : (data?.data ?? [])))
        .catch(() => {});
    }
  }, []);
